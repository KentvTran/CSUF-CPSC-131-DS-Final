...
auto  tail = my_sll.before_begin();
    auto   end  = my_sll.end();
    while(std::next(tail) != end){
      ++tail;
    }

    my_sll.insert_after( tail, object );
    /////////////////////// END-TO-DO (3) ////////////////////////////
  }
...
struct insert_at_front_of_sll
{
  ...
  void operator()( const SomeObject & object ){
      ...
      //my_sll.emplace_after( my_sll.before_begin(), object );
...
struct insert_into_bst
{
  ...
  void operator()( const SomeObject & object )
  {
    ...
    my_bst.insert( { object.upcCode(), object } );
    /////////////////////// END-TO-DO (7) ////////////////////////////
  }
...
struct insert_into_hash_table
{
  ...
  void operator()( const SomeObject & object ){
      ...
      /// SomeObject's base type and review your prior homework class interface. For example, if SomeObject's base type is a
      /// HotelReservation, you might obtain the reservation's key with object.reservationNumber().
      my_hash_table.insert( { object.upcCode(), object } );
      /////////////////////// END-TO-DO (8) ////////////////////////////
  }
...
struct remove_from_back_of_sll
{
  ...
  void operator()( const auto & )
  {
    ...
    if( std::next( my_sll.begin() ) == my_sll.end() ){
      my_sll.clear();
      return;
    }
    auto it = my_sll.before_begin();
    while( std::next( it,2 ) != my_sll .end()){
      ++it;
    }
...
struct remove_from_bst
{
  ...
  void operator()( const SomeObject & object )
  {
    ///////////////////////// TO-DO (15) //////////////////////////////
      ...
      if(!my_bst.empty()){
        auto it = my_bst.find( object.upcCode() );
        if(it != my_bst.end()){
          my_bst.erase( it );
        }
      }
...
struct remove_from_hash_table
{
  ...
  void operator()( const SomeObject & object )
  {
    ...
    {
      auto it = my_hash_table.find( object.upcCode() );
...
struct search_within_vector
{
  ...
  SomeObject * operator()( const auto & )
  {
    ...
    for( auto & object : my_vector){
      if( object.upcCode() == target_key ){
        return &object;
      }
    }
    return nullptr;
